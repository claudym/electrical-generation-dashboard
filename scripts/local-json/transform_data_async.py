...
async def process_date(http_session, current_date_str):
...
async def main():
    ...
    async with aiohttp.ClientSession() as http_session:
        while current_date_obj <= end_date_obj:
            ...
            tasks.append(process_date(http_session, current_date_str))
            current_date_obj += timedelta(days=1)
        
        all_results = await asyncio.gather(*tasks)
    
    all_transformed_data = {k: v for result in all_results if result for k, v in result.items()}
    
    with open('transformed_data_async.json', 'w', encoding='utf-8') as outfile:
