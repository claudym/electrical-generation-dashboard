...
def save_to_sqlite(data, db_name='transformed_data.db'):
    conn = sqlite3.connect(db_name)
    c = conn.cursor()

    # Create table
    c.execute('''CREATE TABLE IF NOT EXISTS energy_data
                 ("id" TEXT PRIMARY KEY, "group" TEXT, "group_plant" TEXT, "company" TEXT, "plant" TEXT, "datetime" TEXT, "energy" TEXT)''')

    for item in data.values():
        c.execute('''INSERT OR REPLACE INTO energy_data 
                     ("id", "group", "group_plant", "company", "plant", "datetime", "energy")
                     VALUES (?, ?, ?, ?, ?, ?, ?)''', 
                     (item['id'], item['group'], item['group_plant'], item['company'], item['plant'], item['datetime'], str(item['energy'])))
...
async def main():
    ...
    save_to_sqlite(all_transformed_data)
    
    print("Data transformation and output to SQLite database complete.")
